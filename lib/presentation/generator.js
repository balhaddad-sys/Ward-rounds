...
function generateObjective(report, interpretation) {
  ...
  if (report?.ocrConfidence !== undefined) {
    const confidencePercent = Math.round(report.ocrConfidence * 100);
    text += `OCR Confidence: ${confidencePercent}%\n\n`;
  }
...
function formatReportType(type) {
  ...
  return types[type] || (type ? type.toUpperCase() : 'Medical Report');
}

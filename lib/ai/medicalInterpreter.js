 ...
 *
 * UPDATED: Now uses GPT-4 for AI interpretation with smart caching to reduce costs
 */

import {
  interpretMedicalReport,
  generateClinicalPearls as generatePearlsWithGPT,
 ...
 */
export async function interpretDocument(ocrResult, options = {}) {
  ...
  const { type, parsed, structuredData } = ocrResult;
  const { useGPT = true, useGoogleScript = false } = options;

  console.log('[AI] Interpreting document...');
  console.log(`[AI] Document type: ${type}, Text length: ${rawText.length} characters`);

  // Try GPT-4 first (best quality)
  if (useGPT && process.env.OPENAI_API_KEY) {
    try {
      console.log('[AI] Using GPT-4 for medical interpretation...');
      const startTime = Date.now();

      const interpretation = await interpretMedicalReport(rawText, type, {
        structuredData,
        parsed
      });

      const processingTime = Date.now() - startTime;
      console.log(`[AI] ✓ GPT-4 interpretation complete in ${processingTime}ms`);
      console.log(`[AI] Found ${interpretation.findings?.length || 0} findings`);
      ...
      console.warn('[AI] GPT-4 failed:', error.message);
      // Continue to fallback options
    }
  } else if (!process.env.OPENAI_API_KEY) {
    console.warn('[AI] OpenAI API key not configured, using fallback methods');
  }

  // Try Google Apps Script if enabled
  if (useGoogleScript) {
    try {
      ...
      console.warn('[AI] Google Script failed:', error.message);
    }
  }

  // Fallback to mock system (basic pattern matching)
  console.log('[AI] Using fallback interpretation system (basic pattern matching)');
  console.log('[AI] ⚠ For better results, configure OpenAI API key in .env.local');
...
export async function generateClinicalPearls(interpretation, documentType = 'general') {
  ...
  // Try GPT-4 first for high-quality pearls
  if (process.env.OPENAI_API_KEY) {
    try {
      console.log('[AI] Using GPT-4 for clinical pearls generation...');
      const startTime = Date.now();

      const result = await generatePearlsWithGPT(interpretation, documentType);

      const processingTime = Date.now() - startTime;
      console.log(`[AI] ✓ Generated ${result.pearls?.length || 0} clinical pearls in ${processingTime}ms`);
      ...
      console.warn('[AI] GPT-4 pearls generation failed:', error.message);
      console.log('[AI] Falling back to basic pearl generation');
    }
  } else {
    console.warn('[AI] OpenAI API key not configured, using basic pearl generation');
  }

  // Fallback to basic pearl generation
  await delay(800);
...
export async function generateTeachingQuestions(interpretation, documentType = 'general') {
  ...
  // Try GPT-4 first for high-quality questions
  if (process.env.OPENAI_API_KEY) {
    try {
      console.log('[AI] Using GPT-4 for teaching questions generation...');
      const startTime = Date.now();

      const result = await generateAttendingQuestions(interpretation, documentType);

      const processingTime = Date.now() - startTime;
      console.log(`[AI] ✓ Generated ${result.questions?.length || 0} teaching questions in ${processingTime}ms`);
      ...
      console.warn('[AI] GPT-4 questions generation failed:', error.message);
      console.log('[AI] Falling back to basic question generation');
    }
  } else {
    console.warn('[AI] OpenAI API key not configured, using basic question generation');
  }

 ...
 */
export async function generateClinicalPearls(interpretation, documentType = 'general', useGoogleScript = true) {
  console.log('[AI] Generating clinical pearls...');

  // Try Google Apps Script first if enabled
  if (useGoogleScript) {
    try {
      console.log('[AI] Attempting Google Apps Script pearls generation...');
      const { generatePearlsWithGoogleScript } = await import('@/lib/services/googleScript');

      const result = await generatePearlsWithGoogleScript(interpretation);

      if (result && result.success && result.pearls) {
        console.log('[AI] ✓ Using Google Apps Script pearls');
        return {
          ...result.pearls,
          source: 'google_script',
          usedAI: true,
      ...
      console.warn('[AI] Google Script pearls failed, falling back to mock:', error.message);
    }
  }

  // Fallback to mock system
  console.log('[AI] Using mock pearls generation');
  await delay(800);
 ...
 */
export async function generateTeachingQuestions(interpretation, documentType = 'general', useGoogleScript = true) {
  console.log('[AI] Generating teaching questions...');

  // Try Google Apps Script first if enabled
  if (useGoogleScript) {
    try {
      console.log('[AI] Attempting Google Apps Script questions generation...');
      const { generateQuestionsWithGoogleScript } = await import('@/lib/services/googleScript');

      const result = await generateQuestionsWithGoogleScript(interpretation);

      if (result && result.success && result.questions) {
        console.log('[AI] ✓ Using Google Apps Script questions');
        return {
          ...result.questions,
          source: 'google_script',
          usedAI: true,
      ...
      console.warn('[AI] Google Script questions failed, falling back to mock:', error.message);
    }
  }

  // Fallback to mock system
  console.log('[AI] Using mock questions generation');
  await delay(800);

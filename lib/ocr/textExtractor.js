 ...
 */
export async function processDocument(file, documentType = 'general') {
  console.log(`[OCR] Processing ${documentType} document...`);

  // For PDFs, we'd need pdf.js or similar, for now just handle images
  if (file.type === 'application/pdf') {
    return {
      text: '[PDF processing not yet implemented - please use images]',
      confidence: 0,
      type: documentType,
      error: 'PDF processing requires additional setup'
    };
  }
  ...
  const parsed = parseByDocumentType(ocrResult.text, documentType);
